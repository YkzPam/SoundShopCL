...
| 2 | Entradas, salidas y validaciones de formularios | `core/forms.py`, vistas del carrito y registro, y sección 2.2 del documento de datos | Cumplido |
...
| 10 | Validación de sintaxis, rutas y visualización con apoyo de IA | `manage.py check`, veintidós pruebas, capturas de `docs/mockups-prototipo.md` y registro de IA | Cumplido |
...
La tabla mide la primera evaluación y no afirma funciones que el proyecto todavía no posee. El formulario nuevo crea una cuenta de sesión para representar la interfaz y validar datos, pero no implementa autenticación persistente. Tampoco existe administración persistente, historial real, pasarela de pago ni seguimiento logístico. Esas capacidades requieren base de datos y corresponden al desarrollo posterior del caso semestral.
